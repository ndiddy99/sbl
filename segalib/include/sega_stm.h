...
/* 無限回ループの指定			*/
#define	STM_LOOP_ENDLESS	LONG_MAX
...
enum StmFad {
    ...
    STM_FAD_CDEND	= LONG_MAX	/* ディスク終了迄読む場合のセクタ数*/
};
...
enum StmSctPos {
    ...
    STM_CDBUF_END	= CDC_SNUM_END	/* 全セクタ数			*/
};
...
enum StmFuncId {
    ...
    STM_SETTRBUF	= 0x316,	/* 取り出し領域の設定		*/
    STM_SETTRFUNC	= 0x317,	/* 取り出し関数の設定		*/
    STM_STARTTRANS	= 0x318,	/* 取り出し関数における転送開始	*/
    STM_SETTRGATE	= 0x319,	/* 取り出しゲートの開閉		*/
    STM_SETTRPARA	= 0x320,	/* 最大取り出しセクタ数の設定	*/
    STM_SETTRFAD	= 0x321,	/* 取り出し開始FADの設定	*/
    STM_SETTRMODE	= 0x322,	/* 取り出しモードの設定		*/
    STM_RESETTRBUF	= 0x323,	/* 取り出し領域のリセット	*/
    STM_GETNUMCDBUF	= 0x324,	/* CDバッファ有効セクタ数の取得	*/
    STM_GETSCTINFO	= 0x325,	/* 読み込みセクタ情報の取得	*/
    STM_GETLENTRBUF	= 0x326,	/* 取り出し領域のデータ数の取得	*/
    STM_ISTRBUFFULL	= 0x327,	/* 取り出し領域フルのチェック	*/
    STM_ISTRANS		= 0x328,	/* 取り出し中のチェック		*/
    ...
    STM_EXECTRANS	= 0x336,	/* ストリームデータの取り出し	*/
...
enum StmErrCode {
    ...
    STM_ERR_PARA	= GFS_ERR_PARA,		/* 不正なモード 	*/
...
typedef struct stm {
    ...
    struct stm	*alias;		/* 実際に操作するストリーム		*/
    struct stm	*sfadlst;	/* 開始FAD昇順リスト			*/
    struct stm	*fltlst;	/* 絞りの接続順リスト			*/
    GfsHn	gfs;		/* ファイルハンドル			*/
    GfsFlow	*flow;		/* 読み込み管理				*/
    Sint32	(*trfunc)(void *obj, struct stm *stm, Sint32 nsct);
                                /* 取り出し関数(StmFuncTrn)		*/
    void	*obj;		/* 取り出し関数の第一引数	　　　　*/
    Uint32	*trsrc;		/* 取り出し元アドレス			*/
    Sint32	adlt;		/* 取り出し元アドレス変化分		*/
    Sint32	trfad;		/* 取り出し開始FAD			*/
    Uint8	atr;		/* アトリビュート			*/
    Uint8	svrstat;
} Stm;
...
typedef void	(*StmErrFunc)(void *obj, Sint32 ec);    /* エラー関数	*/
#define	STM_ERR_NULLFUNC	((StmErrFunc)0)

typedef	void	(*StmCdbufFunc)(void *obj);
#define	STM_CDBUF_NULLFUNC	((StmCdbufFunc)0)  /* バッファ監視関数	*/
...
/* 作業領域サイズの取得			*/
#define	STM_WORK_SIZE(grpmax, stmmax)	(sizeof(StmMng) + sizeof(StmGrp) * grpmax +\
					 sizeof(Stm) * stmmax)
